...
use super::Arc;
use super::utils::{encode_session, build_client};
...
pub async fn tg_request_auth_code_impl(auth_data: TelegramAuthData) -> Result<TelegramAuthResult, TelegramError> {
    let flow_id = AUTH_FLOW_ID.fetch_add(1, Ordering::Relaxed) + 1;
    log::info!("tg_request_auth_code_impl: flow_id={} phone={}", flow_id, auth_data.phone_number);
    ...
    let token = built.client
        ...
        .map_err(|e| {
            ...
            let error_message: String = if (s.contains("dropped") || s.contains("cancelled")) && !s.contains("timeout") {
                format!("Connection interrupted: {}. Please check your internet connection and try again.", e)
            } else if s.contains("timeout") {
                format!("Request timed out: {}. Please check your network connection and try again.", e)
            } else if s.contains("invalid") {
                format!("Invalid phone number format: {}. Please check the number and try again.", e)
            } else if s.contains("flood") {
                format!("Too many requests: {}. Please wait a few minutes before trying again.", e)
            } else if (s.contains("api_id") || s.contains("api")) && !s.contains("invalid") {
                format!("Authentication service error: {}. Please try again later.", e)
            } else {
                format!("Failed to request auth code: {}", e)
            };

            TelegramError { message: error_message }
        })?;

    log::info!("tg_request_auth_code_impl: Successfully requested auth code for phone: {}", auth_data.phone_number);
    ...
    *AUTH_STATE.lock().await = Some(super::AuthState {
    ...
    log::info!("tg_request_auth_code_impl: stored state flow_id={} for phone={}", flow_id, auth_data.phone_number);
...
pub async fn tg_sign_in_with_code_impl(phone_code: String) -> Result<TelegramAuthResult, TelegramError> {
    ...
    if code.is_empty() {
        ...
        return Err(TelegramError { message: "Empty code".into() });
    }
    ...
    match client.sign_in(&token, &code).await {
        ...
        Err(SignInError::InvalidCode) => {
            ...
            Err(TelegramError { message: "Invalid code (check what the frontend is sending)".into() })
        }
        ...
        Err(SignInError::SignUpRequired { terms_of_service: _ }) => {
            ...
            Err(TelegramError { message: "This number requires sign-up (not logged in yet)".into() })
        }

        Err(SignInError::InvalidPassword) => {
            log::warn!("tg_sign_in_with_code_impl: InvalidPassword (this shouldn't happen in code step)");
            let mut guard = AUTH_STATE.lock().await;
            if let Some(state) = guard.as_mut() {
                state.login_token = Some(token);
            }
            Err(TelegramError { message: "Invalid password".into() })
        }
        ...
        Err(SignInError::Other(e)) => {
            ...
            Err(TelegramError { message: format!("Sign-in failed: {e}") })
        }
    }
}

pub async fn tg_generate_qr_code_impl(_app: tauri::AppHandle) -> Result<QrLoginData, TelegramError> {
    ...
    let flow_id = AUTH_FLOW_ID.fetch_add(1, Ordering::Relaxed) + 1;
    log::info!("tg_generate_qr_code_impl: Starting new QR flow (flow_id={})", flow_id);
    ...
    let (token_bytes, raw_expires_field, _token_obj) = match token_result {
        tl::enums::auth::LoginToken::Token(t) => {
            (t.token.clone(), t.expires, t)
        },
        tl::enums::auth::LoginToken::Success(_s) => {
            // Already authorized - this is rare but possible
            log::info!("tg_generate_qr_code_impl: Already authorized (flow_id={})", flow_id);
    ...
    let token_hash = if token_bytes.len() >= 6 {
        format!("{:02x}{:02x}{:02x}{:02x}{:02x}{:02x}", 
            token_bytes[0], token_bytes[1], token_bytes[2],
            token_bytes[3], token_bytes[4], token_bytes[5])
    } else {
        "<short>".to_string()
    };
...
pub async fn tg_poll_qr_login_impl(app: tauri::AppHandle) -> Result<QrPollResult, TelegramError> {
    ...
    let (client, flow_id, _expires_unix, updates_stream) = {
        ...
        let state = match guard.as_ref() {
            Some(s) => s,
            None => return Err(TelegramError { message: "No active QR session".into() }),
        };
        ...
        let qr_state = match &state.qr_state {
            ...
            None => {
                if state.is_migrating {
                    ...
                    return Err(TelegramError { message: "QR flow not started".into() });
                }
            }
        };
        ...
        (state.client.clone(), state.flow_id, qr_state.expires_unix, state.updates.clone())
    };
        ...
        Ok(Some(_upd)) => {
            ...
            match resolve_export_login_token(&client).await? {
                ...
                tl::enums::auth::LoginToken::MigrateTo(m) => {
                    log::info!("tg_poll_qr_login_impl: Finalized to migration DC {}", m.dc_id);
                ...
                tl::enums::auth::LoginToken::Token(t) => {
                    ...
                    if let Some(state) = guard.as_mut() {
                        if let Some(qr) = &mut state.qr_state {
                            if qr.token != t.token {
                                log::info!("tg_poll_qr_login_impl: Token updated during update check");
                                qr.token = t.token.clone();
                                let token_b64 = base64::engine::general_purpose::URL_SAFE_NO_PAD.encode(&t.token);
                                qr.last_token_b64 = token_b64.clone();
                                qr.qr_url = format!("tg://login?token={}", token_b64);
                                qr.expires_unix = new_expires_at;
                                
                                // Notify UI
                                let _ = app.emit("qr-token-updated", serde_json::json!({
                                    "flow_id": flow_id,
                                    "qr_url": qr.qr_url,
...
async fn handle_dc_migration_safe(
    ...
    let (old_home_dc, pool_handle) = {
        ...
        let state = guard
            .as_mut()
            .ok_or_else(|| TelegramError { message: "No active session".into() })?;
        ...
        let import_result = match current_client.invoke_in_dc(current_dc, &import_req).await {
            ...
            Err(e) if e.is("SESSION_PASSWORD_NEEDED") => {
                ...
                return Ok(QrPollResult {
                    ...
                    session_data: None,
                    requires_password: true,
                    message: Some("2-Step Verification enabled. Please enter your password.".to_string()),
                });
            }

            Err(e) => {
                let msg = e.to_string();
                log::error!("handle_dc_migration_safe: Import failed on DC {}: {}", current_dc, msg);
                ...
                return Err(TelegramError {
                    message: format!(
                        "Failed to import login token on DC {}: {}",
                        current_dc, e
                    ),
                });
            }
...
async fn handle_login_success(
    ...
    let user = match s.authorization {
        tl::enums::auth::Authorization::Authorization(a) => match a.user {
            ...
            _ => return Err(TelegramError { message: "Unexpected user type on success".into() }),
        },
        _ => return Err(TelegramError { message: "Sign-up required or other authorization error".into() }),
    };
    ...
    let (encoded_session, user_id) = {
        ...
        let state = match guard.as_mut() {
            Some(s) => s,
            None => return Err(TelegramError { message: "Session lost during success".into() }),
        };
...
pub async fn tg_sign_in_with_password_impl(password: String, db: Database) -> Result<TelegramAuthResult, TelegramError> {
    ...
    if pwd.is_empty() {
        ...
        return Err(TelegramError { message: "Empty password".into() });
    }

    log::info!(
        "tg_sign_in_with_password_impl: attempting check_password (phone={:?}, flow_id={})",
        stored_phone, flow_id
    );
    ...
    match check {
        Ok(_user) => {
            ...
            let phone_for_db = stored_phone
                .clone()
                .or_else(|| me.phone().map(|p| if p.starts_with('+') { p.to_string() } else { format!("+{}", p) }))
        ...
        Err(e) => {
            log::error!("tg_sign_in_with_password_impl: check_password FAILED: {}", e);
            ...
            if msg.contains("password_hash_invalid") || msg.contains("invalid") {
                return Err(TelegramError { message: "Wrong 2FA password.".into() });
            }

            Err(TelegramError { message: format!("Password authentication failed: {e}") })
        }
    }
}
...
pub async fn tg_cancel_qr_login_impl() -> Result<bool, TelegramError> {
    ...
    if let Some(state) = guard.as_mut() {
        if state.qr_state.is_some() {
            log::info!("tg_cancel_qr_login_impl: Cancelling QR flow (flow_id={})", state.flow_id);
            state.qr_state = None;
            return Ok(true);
        }
    }
...
async fn handle_already_authorized(
    ...
    log::info!("handle_already_authorized: Already logged in as user_id={}", user.raw.id());
